...
class PrivateMessages
{
    ...
    public function index($req, $res, $args)
    {
        ...
        if ($action = Input::post('action')) {
            switch ($action) {
                ...
                case 'unread':
                    ...
                    break;
                default:
                    return Router::redirect(Router::pathFor('Conversations.home',
        ...
        $paging_links = Url::paginate($num_pages, $p, Router::pathFor('Conversations.home',
                ['id' => $args['inbox_id']]) . '/#');

        // Make breadcrumbs
        $this->crumbs[Router::pathFor('Conversations.home',
            ['inbox_id' => $args['inbox_id']])] = $this->inboxes[$args['inbox_id']]['name'];
        ...
        View::setPageInfo(array(
                'title' => array(Utils::escape(ForumSettings::get('o_board_title')),
                    __('PMS', 'private_messages'), $this->inboxes[$args['inbox_id']]['name']),
                'admin_console' => true,
                'inboxes' => $this->inboxes,
                'current_inbox_id' => $args['inbox_id'],
                'paging_links' => $paging_links,
                'rightLink' => ['link' => Router::pathFor('Conversations.send'),
                    'text' => __('Send', 'private_messages')],
                'conversations' => $this->model->getConversations($args['inbox_id'], $uid,
                    User::get()['disp_topics'], $start_from)
            )
        )
    ...
    public function delete($req, $res, $args)
    {
        ...
        if (Input::post('delete_comply')) {
            ...
            return Router::redirect(Router::pathFor('Conversations.home'),
                __('Conversations deleted', 'private_messages'));
        } else {
    ...
    public function move($req, $res, $args)
    {
        ...
        if (Input::post('move_comply')) {
            ...
            if ($this->model->move($topics, $move_to, $uid)) {
                return Router::redirect(Router::pathFor('Conversations.home', ['inbox_id' => $move_to]),
                    __('Conversations moved', 'private_messages'));
            } else {
        ...
        if (Request::isPost()) {
            ...
            if (Input::post('preview')) {
                ...
                if (!is_null($data['preview']) && User::get()['last_post'] != '' &&
                    (Container::get('now') - User::get()['last_post']) < Container::get('prefs')->get(User::get(),
                        'post.min_interval')) {
                ...
                if ($args['tid']) {
                    ...
                    if ($conv) {
                        ...
                        if ($msg_id = $this->model->addMessage($msg_data, $args['tid'])) {
                            return Router::redirect(Router::pathFor('Conversations.home'),
                                sprintf(__('Reply success', 'private_messages'), $conv->subject));
                        }
                    } else {
                        // Add message in conversation + add receiver (create new conversation)
                        if ($msg_id = $this->model->addMessage($msg_data, $args['tid'],
                            array($user->id, User::get()->id))) {
                            return Router::redirect(Router::pathFor('Conversations.home'),
                                sprintf(__('Send success', 'private_messages'), $user->username));
                        }
                    }
                } else {
    ...
    public function show($req, $res, $args)
    {
        ...
        $start_from = User::get()['disp_topics'] * ($p - 1);
        $paging_links = Url::paginate($num_pages, $p, Router::pathFor('Conversations.show',
                ['tid' => $args['tid']]) . '/#');

        $this->inboxes = $this->model->getInboxes(User::get()->id);

        $this->crumbs[Router::pathFor('Conversations.home',
            ['inbox_id' => $conv['folder_id']])] = $this->inboxes[$conv['folder_id']]['name'];
        $this->crumbs[] = __('My conversations', 'private_messages');
        $this->crumbs[] = $conv['subject'];
        Utils::generateBreadcrumbs($this->crumbs, array(
        ...
        View::setPageInfo([
                ...
                'cur_conv' => $conv,
                'rightLink' => [
                    'link' => Router::pathFor('Conversations.reply',
                    ['tid' => $conv['id']]), 'text' => __('Reply', 'private_messages')
                ],
    ...
    public function blocked($req, $res, $args)
    {
        ...
        if (Input::post('add_block')) {
            ...
            if (empty($errors)) {
                ...
                return Router::redirect(Router::pathFor('Conversations.blocked'),
                    __('Block added', 'private_messages'));
            }
            ...
            return Router::redirect(Router::pathFor('Conversations.blocked'),
                __('Block removed', 'private_messages'));
        }
        ...
        View::setPageInfo(array(
                'title' => array(Utils::escape(ForumSettings::get('o_board_title')),
                    __('PMS', 'private_messages'), __('Blocked Users', 'private_messages')),
                'admin_console' => true,
                'errors' => $errors,
                'username' => $username,
    ...
    public function folders($req, $res, $args)
    {
        ...
        if (Input::post('add_folder')) {
            ...
            if (empty($errors)) {
                ...
                return Router::redirect(Router::pathFor('Conversations.folders'),
                    __('Folder added', 'private_messages'));
            }
            ...
            if (empty($errors)) {
                ...
                if ($this->model->updateFolder(User::get()->id, $id, $update))
                    return Router::redirect(Router::pathFor('Conversations.folders'),
                        __('Folder updated', 'private_messages'));
                else
            ...
            if ($this->model->removeFolder(User::get()->id, $id))
                return Router::redirect(Router::pathFor('Conversations.folders'),
                    __('Folder removed', 'private_messages'));
            else
                throw new  RunBBException(__('Error'), 403);
        }
